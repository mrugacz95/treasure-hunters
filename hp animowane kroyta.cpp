...
int zycie(int hp)
{
    ...
    if(((hp==i)||(hp>i))&&!(hp==0)){ 
               SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 192);
                    switch(i) 
                    {
                     case 50: std::cout<<(hp-hp%10)/10; break;
                     case 60: std::cout<<hp%10; break;
                     default: SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 204); std::cout<<static_cast<char>(219); break;
                     } 
              ...
              SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
                                  switch(i) 
                    {
                     ...
                     case 50: std::cout<<(hp-hp%10)/10; break;
                     case 60: std::cout<<hp%10; break;
                     default: std::cout<<static_cast<char>(176); break;
                     } 
              }
    }
     ...
     std::cout <<"                                  " << static_cast<char>(186);
             for(int j=0;j<10;j++)
                ...
                std::cout << static_cast<char>(186);
                    switch(i)
                     {
                      case 4: std::cout<<"    HP: "; zycie(wynik);
                           break;
...
if(znak==27){break;}
if(znak==13){ wypelnij(0); rysuj(); wygenerowanych++; }
if(znak==0||konsola==true){ znak=getch(); if(znak==59){
             //KONSLOA
             std::cout << "ANIMKORYT = v1.4"<<std::endl;
    ...
    if(nplansza[xludzik][yludzik+1]!=static_cast<char>(219)) {std::cout<<"> ";}
        znak=getch();
        if(znak==107){ konsola = true;}
        
        //koniec konsoli
            } }
if(znak==224)
{
                    gotoxy(yludzik+36, xludzik+2);
   ...
   gotoxy(yludzik+36, xludzik+2);
   std::cout << static_cast<char>(173);  
}
gotoxy(47, 12);
znak=static_cast<char>(0);

if(nplansza[xludzik][yludzik]==static_cast<char>(193)){ wynik-=5; gotoxy(47, 6); std::cout<<"    HP: "; zycie(wynik); gotoxy(52, 12);} 
if(nplansza[xludzik][yludzik]==static_cast<char>(29)){ wynik++; invert=true;}
if(nplansza[xludzik][yludzik]==static_cast<char>(2)){ wynik-=3; ilosckolcow-=5;}
if(wynik<0){ wynik=0; std::cout<<std::endl; std::cout<<"Masz mniej punktow niz 0 - koniec tej farsy. "; ilosckolcow=0; przyszedlz=0; break;} 
if(wynik==wygrana){system("cls"); std::cout<< "Brawo zdobyles duzo puktow"<<std::endl; return; }
if((xludzik==xskarb)&&(yludzik==yskarb)){ zebrany=true; gotoxy(47, 7); std::cout<<"    COLLECTED"; gotoxy(47, 12); }
